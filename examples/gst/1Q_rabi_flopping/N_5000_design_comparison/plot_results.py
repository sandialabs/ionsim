...
if __name__ == '__main__':
    ...
    DIR_PREFIX = ""
    folders = ["N_5000_naive", "N_5000_design2"]
    ...
    for i, f in enumerate(folders):
        ...
        plt.errorbar(num_circuits, gate_error, std_err, marker = 'o', linewidth = 0.5, markersize = 6, color = colors[i], label='GST')
        ...
        plt.plot(np.exp(X[start_indx:end_indx]), np.exp(coefficients[0]*X[start_indx:end_indx] + coefficients[1]), linestyle = 'solid', color = colors[i], linewidth = 1.5, label = r'Fit: $m = ' + str(np.round(coefficients[0].real, 3)) + '$')
    ...
    plt.xscale('log')
    plt.yscale('log')
    plt.savefig('X_pi8_err_N5000_shots_design_comparison.pdf', dpi=300)
    plt.show()
