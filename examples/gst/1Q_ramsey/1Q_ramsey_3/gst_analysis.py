...
############ Option 1 for POVMs: Give a dictionary of models (callables) ############
 ...
 #
    ...
    ideal_gate_set['Gxpi2:0'] = X_pi_2_co_prop(0.)
    ...
    ## TODO: Add initial guesses for prep and measure models 
    #parameters_guess = {"prep" : {"prob_wrong_prep" : 0.005}, "POVM" : {"prob_false_0" : 0.005, "prob_false_1" : 0.0025}, 
    #    "Gxpi2:0" : {"spin_flip_rate" : 1.}, "idle" : {"energy_shift" : 5*2.*np.pi, "spin_dephasing_rate" : 2000.}}
    parameters_guess = {"Gxpi2:0" : {"spin_flip_rate" : 1.}, "idle" : {"energy_shift" : 5*2.*np.pi, "spin_dephasing_rate" : 2000.}}

    G = sm.ParsedGate.from_string("Gxpi2:0")
    idle_G = sm.ParsedGate("idle", ())
    parameter_bounds = {
        "prep" : {"prob_wrong_prep" : (0., 1.)}, 
        "POVM" : {"prob_false_0" : (0., 1.), "prob_false_1" : (0., 1.)},
        G : {"spin_flip_rate" : (0., 10.)},
        idle_G : {"energy_shift" : (0., 1000.), "spin_dephasing_rate" : (0, None)}
    } 
