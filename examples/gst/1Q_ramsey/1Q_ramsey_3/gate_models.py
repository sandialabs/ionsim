...
def prep_state_function(prob_wrong_prep: float): 
    ...
    rho_q1[0,0] = (1. - prob_wrong_prep)
    rho_q1[1,1] = prob_wrong_prep
...
def E0_1Q(prob_false_0:float, prob_false_1: float):
    M = np.zeros((2,2), dtype=complex)
    M[0,0] = (1. - prob_false_0)
    M[1,1] = prob_false_1
    return M

def E1_1Q(prob_false_0:float, prob_false_1: float):
    M = np.zeros((2,2), dtype=complex)
    M[0,0] = prob_false_0
    M[1,1] = (1. - prob_false_1)
    return M


def POVM_models(prob_false_0: float, prob_false_1: float) -> dict:
    """ Dictionary of POVMs evaluated at the function parameters: """ 
    POVMs = {}

    # 0
    M0 = E0_1Q(prob_false_0, prob_false_1)
    operator = sm.EnergyShiftOperator.from_matrix(basis, M0)
    POVMs["0"] = operator.superbra 

    # 1
    M1 = E1_1Q(prob_false_0, prob_false_1)
...
def X_pi_2_co_prop(spin_flip_rate: float): 
    """ Process matrix Error model for X_pi/8 (small rotation angle) gate for Rabi flopping
