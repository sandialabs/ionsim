...
def main():
    ...
    if Path(gst_circuit_filename).exists():
        ...
        #powers = [1, 2, 4, 8, 10, 16, 24, 32]
        #powers = [1, 2, 4, 8, 10, 16, 24, 32]
        #gst_circuit_planner = ism.GSTCircuitPlanner(gate_names, qubit_indices, germ_powers = powers)
        ...
        # Write design file  
        gst_circuit_planner.write_circuit_design('./circuit_design.yml')
        #sys.exit(0)
    ...
    gate_mappings = { 'Gxpi2' : X_pi2_state_propagator, 'Gypi2' : Y_pi2_state_propagator, 'idle' : idle_state_propagator} # these could be python modules
    ...
    if not Path(circuit_simulation_output_file).exists():
    ...
    for circuit in gst_circuits:
        ...
        N_shots = 2000   # I think this is just chosen by the experiment, rather than being related to the number of trajectories in a simulation of one gate or circuit. 
        #N_shots = 500    # I think this is just chosen by the experiment, rather than being related to the number of trajectories in a simulation of one gate or circuit. 
