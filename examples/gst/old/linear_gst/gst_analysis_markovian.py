...
def main():
    ...
    def X_pi2(X_rot: float, phase_mean: float, phase_std_deviation: float):
    ...
    def Y_pi2(Y_rot: float, phase_mean: float, phase_std_deviation: float):
    ...
    if use_interpolated_gates:
        ...
        if Path(X_pi2_fname).exists():
            ...
            Xpi2_model_sig = inspect.signature(X_pi2)
            ...
            Xpi2_gate_interpolant = sm.GateInterpolant.from_process_matrix_function(X_pi2, parameter_grid, basis, 'Gxpi2')
        ...
        if Path(Y_pi2_fname).exists():
            ...
            # Build gate interpolant from process matrix functions specified above 
            Ypi2_model_sig = inspect.signature(Y_pi2)
            ...
            Ypi2_gate_interpolant = sm.GateInterpolant.from_process_matrix_function(Y_pi2, parameter_grid, basis, 'Gypi2')
    ...
    ideal_gate_set = {}
    ideal_gate_set['Gxpi2'] = X_pi2(0., 0., 0.) 
    ideal_gate_set['Gypi2'] = Y_pi2(0., 0., 0.)  
    ideal_gate_set['idle'] = idle(0.) 
    ideal_gate_set['prep'] = ideal_rho_prep 
    ideal_gate_set['POVM'] = ideal_POVM_effects 


    ## 07/18 TODO: Test circuit function 

    import inspect 
    X_pi2_sig = inspect.signature(X_pi2)
    parameters = list(X_pi2_sig.parameters.keys())
    values = [0., 0., 0.]
    parameters = dict(zip(parameters, values))
    X_pi2_gate = sm.Gate.from_process_matrix_function(basis, X_pi2, parameters) 


    Y_pi2_sig = inspect.signature(Y_pi2)
    parameters = list(Y_pi2_sig.parameters.keys())
    print(parameters)
    values = [0., 0., 0.]
    parameters = dict(zip(parameters, values))
    Y_pi2_gate = sm.Gate.from_process_matrix_function(basis, Y_pi2, parameters) 

    idle_gate = sm.Gate.from_process_matrix_function(basis, idle, {'theta' : 0.}) 
...
#            parameters: dict[str, float], noise: Noise | None = None):
    gate_list = [X_pi2_gate, X_pi2_gate, Y_pi2_gate, idle_gate, idle_gate, Y_pi2_gate]
    example_circuit = sm.Circuit.from_gates(gate_list)
    #print(example_circuit.process_matrix_function)
    # Test circuit process matrix function and probability function
    projector_0 = sm.EnergyShiftOperator.from_matrix(basis, sm.Pauli.projector_0) 
    projector_1 = sm.EnergyShiftOperator.from_matrix(basis, sm.Pauli.projector_1) 

    outcome0_prob_function = example_circuit.build_outcome_probability_function(ideal_rho_prep, projector_0)
    # Test evaluation
    #print(outcome0_prob_function)
    pm_circ_sig = inspect.signature(outcome0_prob_function)
    parameters = list(pm_circ_sig.parameters.keys())
    print(parameters)
    #test_output = outcome0_prob_function(0.015, 0.001, 0.05, 0.02, 0.0025, 0.05, 0.1)
    # Specify circuit parameter inputs:
    X_pi2_parameters = {"X_pi2__phase_mean" : 0.025, "X_pi2__X_rot" : 0.05, "X_pi2__phase_std_deviation" : 0.1} 
    Y_pi2_parameters = {"Y_pi2__phase_mean" : 0.025, "Y_pi2__Y_rot" : 0.05, "Y_pi2__phase_std_deviation" : 0.1} 
    idle_parameters = {"idle__theta" : 0.003} 
    
    circuit_function_args = X_pi2_parameters | Y_pi2_parameters | idle_parameters
    test_output = outcome0_prob_function(**circuit_function_args)

    print(f"Probability of outcome |0>: {test_output}")

    # Test outcome probabilities function: 
    outcome_probs_function = example_circuit.build_outcome_probabilities_function(ideal_rho_prep, [projector_0, projector_1])
    test_output = outcome_probs_function(**circuit_function_args)
    
    print(f"Probabilities of outcomes |0> and |1>: {test_output}")


    sys.exit(0)


    GST_analyzer = sm.GateSetTomography(basis, prep_state_function, POVM_models, parsed_circuits, ism_gate_dictionary, parameter_bounds, gst_circuit_design, ideal_gate_set)
    ...
    solver_results = GST_analyzer.solve_for_gate_parameters(theta_guess, 'linear')
    ...
    ideal_gate_set['prep'] = ideal_rho_prep 
    ideal_gate_set['POVM'] = ideal_POVM_effects 
    ideal_gate_set['Gxpi2'] = X_pi2(0.015, 0.025, 0.01) 
    ideal_gate_set['Gypi2'] = Y_pi2(0.0075, 0.125, 0.05)  
